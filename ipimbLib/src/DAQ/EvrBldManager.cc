...
EvrBldManager::EvrBldManager(EvgrBoardInfo<Evr> &erInfo, unsigned opcode,float evrPulseDelay, EvrBldServer& evrBldServer, 
                             CfgClientNfs** cfgIpimb, unsigned nIpimbBoards, unsigned* interfaceOffset) :
                             _er(erInfo.board()), _opcode(opcode),_evrPulseDelay(evrPulseDelay), _evrBldServer(evrBldServer),
...
void EvrBldManager::configure() { 
  ...
  int delay=(int)(_evrPulseDelay*tickspersec);
  printf("Programmed EVR Pulse Delay = % f  Sec \n",_evrPulseDelay );
